...
class DemandErrorServiceImpl(
    ...
    override fun handle(demand: DemandRejectErrorOnDemandDto): Mono<Unit> =
        demandStatusRepository.existsByDemandId(demand.demandId).flatMap { isExists ->
            if(isExists) demandStatusRepository.findByDemandId(demand.demandId)
            else DemandStatus(id = 0, demandId = demand.demandId, status = demand.originStatus).toMono()
        }.map{ demandStatus -> demandStatus.copy(status = demand.originStatus) }
